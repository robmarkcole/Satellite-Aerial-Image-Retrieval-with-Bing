    ...
    def max_resolution_imagery_retrieval(self):
        ...
        for levl in range(TileSystem.MAXLEVEL, 0, -1):
            ...
            if abs(pixelX1 - pixelX2) * abs(pixelY1 - pixelY2) > IMAGEMAXSIZE:
                print(
                    "Current level {} results an image exceeding the maximum image size {}, will SKIP".format(
                        levl, IMAGEMAXSIZE
                    )
                )
                continue
            ...
            for tileY in range(tileY1, tileY2 + 1):
                ...
                string_remaining_time = str(timedelta(seconds=avg_download_time)).split(
                    ".", 1
                )[0]
            ...
            leftup_cornerX, leftup_cornerY = TileSystem.tileXY_to_pixelXY(
                tileX1, tileY1
            )
            ...
            print(
                "Finish the aerial image retrieval, store the image aerialImage_{0}.jpeg in folder {1}".format(
                    levl, self.tgtfolder
                )
            )
    ...
    def horizontal_retrieval_and_stitch_image(
        ...
        for tileX in range(tileX_start, tileX_end + 1):
            ...
            if self.is_valid_image(image):
                ...
                # print(quadkey)
                print(
                    "Cannot find tile image at level {0} for tile coordinate ({1}, {2})".format(
                        level, tileX, tileY
                    )
                )
                return False, None
...
def main():
    ...
    try:
        ...
        sys.exit(
            "Diagonal (Latitude, Longitude) coordinates of the bounding box must be input"
        )
    if len(args) != 4:
        sys.exit(
            "Please input Latitude, Longitude coordinates for both upper-left and lower-right corners!"
        )
    ...
    if imgretrieval.max_resolution_imagery_retrieval():
        ...
        print(
            "Cannot retrieve the desired image! (Possible reason: expected tile image does not exist.)"
        )
